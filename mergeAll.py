...
for videoFile in glob.iglob('*-video.*'):
    ...
    print(finalFile)
    if srtFile != '':
        sp.run(['ffmpeg', '-y', '-i', videoFile, '-i', audioFile, '-i', srtFile, '-map', '0:0', '-map', '1:0', '-map', '2:0',
            '-vcodec', 'copy', '-acodec', 'copy', '-scodec', 'copy', finalFile], stdout=sp.PIPE, stderr=sp.PIPE)
    else:
        sp.run(['ffmpeg', '-y', '-i', videoFile, '-i', audioFile, '-map', '0:0', '-map', '1:0', '-vcodec', 'copy', '-acodec', 'copy', finalFile],
